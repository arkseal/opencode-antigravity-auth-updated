...
export function resolveModelWithTier(
  ...
  if (isEffectiveGemini3) {
    const effectiveThinkingLevel =
      ((resolvedModel === GEMINI_37_FLASH_TIERED_MODEL || resolvedModel === GEMINI_38_FLASH_TIERED_MODEL) && tier === "minimal")
        ? "low"
        : tier;
    return {
      actualModel: resolvedModel,
  ...
  const thinkingBudget = (tier in budgets) ? budgets[tier as keyof typeof budgets] : undefined;
