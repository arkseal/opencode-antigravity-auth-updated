...
describe("updateOpencodeConfig", () => {
  ...
  test("creates new config with default structure when file does not exist", async () => {
    ...
    expect(writtenConfig.plugin).toContain("opencode-antigravity-auth-updated@latest");
  ...
  test("preserves $schema and other top-level config keys", async () => {
    const existingConfig = {
      $schema: "https://opencode.ai/config.json",
      plugin: ["opencode-antigravity-auth-updated@latest", "other-plugin"],
      theme: "dark",
  ...
  test("does not duplicate plugin if different version present", async () => {
    const existingConfig = {
      plugin: ["opencode-antigravity-auth-updated@beta", "other-plugin"],
      provider: {},
    };
    fs.writeFileSync(configPath, JSON.stringify(existingConfig));
    ...
    expect(writtenConfig.plugin).toContain("opencode-antigravity-auth-updated@beta");
  });
