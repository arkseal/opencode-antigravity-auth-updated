...
export class AccountManager {
    ...
    getCurrentOrNextForFamily(family, model, strategy = 'sticky', headerStyle = 'antigravity', pidOffsetEnabled = false, softQuotaThresholdPercent = 100, softQuotaCacheTtlMs = 10 * 60 * 1000, excludeIndices) {
    ...
    getNextForFamily(family, model, headerStyle = "antigravity", softQuotaThresholdPercent = 100, softQuotaCacheTtlMs = 10 * 60 * 1000, excludeIndices) {
    ...
    updateQuotaCache(accountRef, quotaGroups) {
        const account = typeof accountRef === "number"
            ? this.accounts[accountRef]
            : this.accounts.find(a => a.parts.refreshToken === accountRef);
        if (account) {
     ...
     */
    shouldRefreshAllQuotas(family, thresholdPercent = 100, cacheTtlMs = 10 * 60 * 1000, model) {
