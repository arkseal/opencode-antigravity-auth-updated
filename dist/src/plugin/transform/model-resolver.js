 ...
 */
export function resolveAntigravityGemini31ProBackendModel(model, thinkingLevel) {
...
export function resolveAntigravityGemini36FlashBackendModel(model, thinkingLevel) {
    ...
    const level = (thinkingLevel ?? match[1] ?? "low").toLowerCase();
    if (level === "high")
        return GEMINI_36_FLASH_HIGH_MODEL;
    if (level === "medium")
        return GEMINI_36_FLASH_MEDIUM_MODEL;
...
/**
 * Resolves antigravity-gemini-3.8-flash to Cloud Code backend model ids.
 * Maps to gemini-3.8-flash-tiered with low/medium/high thinking levels supported by Google's backend.
 */
export function resolveAntigravityGemini38FlashBackendModel(model, _thinkingLevel) {
    const modelWithoutQuota = model.replace(QUOTA_PREFIX_REGEX, "");
    const match = modelWithoutQuota.match(GEMINI_38_FLASH_REGEX);
    if (!match) {
        return undefined;
    }
    return GEMINI_38_FLASH_TIERED_MODEL;
}
/**
...
export function resolveModelWithTier(requestedModel, options = {}) {
    ...
    if (isEffectiveGemini3) {
        const effectiveThinkingLevel = ((resolvedModel === GEMINI_37_FLASH_TIERED_MODEL || resolvedModel === GEMINI_38_FLASH_TIERED_MODEL) && tier === "minimal")
            ? "low"
            : tier;
        return {
            actualModel: resolvedModel,
