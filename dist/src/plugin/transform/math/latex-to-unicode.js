 ...
 */
export function transformMathExpression(expr) {
    let res = expr;
 ...
 */
function hasMathIndicators(expr) {
    ...
    if (/[a-zA-Z0-9]\s*[=+\-*/<>≤≥≠≈]\s*[a-zA-Z0-9]/.test(expr))
        return true;
    return false;
}
/**
 * Checks if a $ occurrence looks like currency (e.g. $10, $5.99) or shell var ($VAR, ${VAR}).
 */
function isFalsePositiveDollar(text, index, inner) {
    // Escaped \$
    ...
    if (afterClosing && /[0-9]/.test(afterClosing) && !hasMathIndicators(inner))
        return true;
    return false;
}
/**
