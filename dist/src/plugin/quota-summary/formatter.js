import { shortEmail, progressBar, formatDuration } from "./utils.js";
export function formatQuotaReport(results, accounts) {
    const parts = [];
    // Collect errors
        ...
        for (const groupName of groupNames) {
            ...
            for (let i = 0; i < accounts.length; i++) {
                ...
                for (const b of group.buckets) {
                    const label = (b.displayName || b.bucketId || "Limit")
    ...
    // Local Cache Status
    const now = Date.now();
    const accountsWithCache = accounts.filter((a) => a.rateLimitResetTimes && Object.keys(a.rateLimitResetTimes).length > 0);
