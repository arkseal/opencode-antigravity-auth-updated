...
import { ANTIGRAVITY_DEFAULT_PROJECT_ID, ANTIGRAVITY_ENDPOINT, ANTIGRAVITY_ENDPOINT_FALLBACKS, ANTIGRAVITY_ENDPOINT_PROD, ANTIGRAVITY_PROVIDER_ID, getAntigravityHeaders, } from "./constants.js";
...
import { clearAccounts, getStoragePath, loadAccounts, saveAccounts, saveAccountsReplace } from "./plugin/storage.js";
...
import { checkAccountsQuota, fetchAvailableModels, triggerAsyncQuotaRefreshForAll } from "./plugin/quota.js";
...
function mergeAbortSignals(...signals) {
    const activeSignals = signals.filter((s) => s !== undefined);
...
function isRetryableAgySdkCapacityStatus(status) {
    return status === 429 || status === 503 || status === 529;
}
function defaultRetryMsForConfig(config) {
    return (config.default_retry_after_seconds ?? 60) * 1000;
}
async function tryFetchWithAgySdkCredentials(input, init, credentials, fallbackRetryAfterMs) {
...
async function modelsFromAgySdkCredentials(config, auth) {
    if (!config.model_discovery.enabled || !config.model_discovery.gemini_api)
        return {};
...
async function modelsFromOAuthAuth(config, auth, client, providerId) {
    if (!config.model_discovery.enabled || !config.model_discovery.antigravity || !auth || !isOAuthAuth(auth))
        return {};
...
function hasProviderModelRuntimeShape(model) {
    return !!model
...
function modalitiesToCapabilities(model, existing) {
    const modalities = model.modalities;
    const existingCapabilities = existing?.capabilities;
    const existingInput = existingCapabilities?.input;
    const existingOutput = existingCapabilities?.output;
...
function costForProviderModel(model, existing) {
...
function normalizeProviderHookModels(providerId, provider, definitions, existingModels = provider.models ?? {}) {
    const normalized = {};
...
async function tryAgySdkFallbackForRequest(input, init, config, credentials, urlString) {
    if (!config.agy_sdk.api_key_fallback || credentials.length === 0 || !isAgySdkSupportedRequest(urlString)) {
        return null;
    }
    return tryFetchWithAgySdkCredentials(input, init, credentials, defaultRetryMsForConfig(config));
}
            ...
            loader: async (getAuth, provider) => {
                ...
                if (!isOAuthAuth(auth)) {
                    ...
                    return {
                        ...
                        async fetch(input, init) {
                            ...
                            const response = await tryFetchWithAgySdkCredentials(input, init, latestCredentials, (config.default_retry_after_seconds ?? 60) * 1000);
                            if (response)
                                return response;
                            return fetch(input, init);
                        },
                ...
                return {
                    ...
                    async fetch(input, init) {
                        ...
                        const reloadProviderState = async (reason) => {
                        ...
                        const triedSwitchIndices = new Set();
                        ...
                        while (true) {
                            ...
                            if (++loopGuard > Math.max(50, accountCount * 8)) {
                                ...
                                throw lastError || new Error(`Antigravity request routing did not converge for ${model ?? family}. ` +
                            ...
                            if (family === "gemini" &&
                                ...
                                const response = await tryFetchWithAgySdkCredentials(input, init, agySdkCredentials, (config.default_retry_after_seconds ?? 60) * 1000);
                            ...
                            let account = accountManager.getCurrentOrNextForFamily(family, model, config.account_selection_strategy, preferredHeaderStyle, config.pid_offset_enabled, config.soft_quota_threshold_percent, softQuotaCacheTtlMs, triedSwitchIndices);
                            if (!account && allowQuotaFallback) {
                                const alternateHeaderStyle = preferredHeaderStyle === "antigravity" ? "gemini-cli" : "antigravity";
                                account = accountManager.getCurrentOrNextForFamily(family, model, config.account_selection_strategy, alternateHeaderStyle, config.pid_offset_enabled, config.soft_quota_threshold_percent, softQuotaCacheTtlMs, triedSwitchIndices);
                                if (account) {
                            ...
                            if (!account) {
                                ...
                                if (accountCount > 0 && triedSwitchIndices.size >= accountCount) {
                                    ...
                                    throw lastError || new Error(`All ${accountCount} Antigravity account(s) are rate-limited for ${model ?? family}. ` +
                                        "Run `opencode auth login` to add accounts or wait for quota reset.");
                                }
                                ...
                                try {
                                    ...
                                    if (error instanceof AntigravityTokenRefreshError && error.code === "invalid_grant") {
                                        ...
                                        if (accountManager.getAccountCount() === 0) {
                                            ...
                                            const fallback = await tryAgySdkFallbackForRequest(input, init, config, agySdkCredentials, urlString);
                                            if (fallback)
                                                return fallback;
                            ...
                            while (!shouldSwitchAccount) {
                                ...
                                for (let i = 0; i < ANTIGRAVITY_ENDPOINT_FALLBACKS.length; i++) {
                                    ...
                                    try {
                                        const prepared = await prepareAntigravityRequest(input, init, accessToken, projectContext.effectiveProjectId, currentEndpoint, headerStyle, forceThinkingRecovery, {
                                        ...
                                        if (response.status === 429 || response.status === 503 || response.status === 529) {
                                            ...
                                            const agySdkFallbackResponse = await tryAgySdkFallbackForRequest(input, init, config, agySdkCredentials, urlString);
                                        ...
                                        if (error instanceof Error && (error.name === "AbortError" || error.name === "TimeoutError")) {
                                            ...
                                            await showToast(`⏳ Account stuck (${actualTimeoutSec}s). Rotating to next available...`, "warning");
                                            shouldSwitchAccount = true;
                                            lastError = error;
                                            break;
                                        }
                            ...
                            while (accounts.length < MAX_OAUTH_ACCOUNTS) {
                                ...
                                try {
                                    ...
                                    const errorCode = (typeof error === "object" &&
                                        ...
                                        ? error.code
                                        : undefined;
                                    const isLockError = errorCode === "ELOCKED" ||
                                        errorCode === "EEXIST" ||
                                    ...
                                    const lockGuidance = isLockError
                                        ...
                                        : "";
                                    const accountPersistenceWarning = `failed to persist account data (${reason}).${lockGuidance}`;
